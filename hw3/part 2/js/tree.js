...
class Tree {
    ...
    createTree(treeData) {
        ...
        var nodes = d3.hierarchy(this.data, d => d.children);
        ...
        updateTree(row) {
            ...
            this.nodeSelection.filter(d=>{
                ...
                if (row.value.Result.label != 'Group' && row.value.type == 'game'){
                    var oponents = row.unique.split('_');
                ...
                return d.parent && d.parent.data.data.Team == row.key &&
                d.data.data.Team == d.parent.data.data.Team ||
                d.data.data.Team == row.key;
            }).classed('selectedLabel', true);

            var links = this.linkSelection.filter(d=>{
